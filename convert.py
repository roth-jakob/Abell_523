...
#x=np.genfromtxt('point_sources.stat').T[8]
#y=np.genfromtxt('point_sources.stat').T[9]
x=np.genfromtxt('point_sources.stat').T[8]
y=np.genfromtxt('point_sources.stat').T[9]
z=np.zeros(len(x))
w=np.zeros(len(x))
...
w = wcs.WCS('A523_SP06_08_UV_ALL_HB_I.fits')

ra_list=[]
dec_list=[]

p=open('absolute_coordinates.dat', 'w')
for i in range(0, len(x)):
    ...
    p.write(str(w.all_pix2world([[x[i],y[i],0,0]], 1)[0][0])+' '+str(w.all_pix2world([[x[i],y[i],0,0]], 1)[0][1])+'\n')
p.close()
